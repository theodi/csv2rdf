    ...
    private
      ...
      def transform_data(data, sourceRow)
        ...
        @columns.each_with_index do |column,i|
          unless column.suppress_output
            ...
            subject = column.about_url ? RDF::Resource.new(URI.join(@source_url, column.about_url.expand(values)).to_s) : row_subject
            subjects << subject
      ...
      def property(column, values)
        if column.property_url
          ...
          url = URI.join(@source_url, "##{URI.escape(url, Regexp.new("[^A-Za-z0-9_.]"))}")
        end
